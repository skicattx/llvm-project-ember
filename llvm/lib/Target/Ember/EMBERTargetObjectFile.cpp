...
SSThreshold("EMBER-ssection-threshold", cl::Hidden,
            cl::desc("Small data and bss section threshold size (default=8)"),
            cl::init(8));

static cl::opt<bool>
LocalSData("mlocal-sdata", cl::Hidden,
           cl::desc("EMBER: Use gp_rel for object-local data."),
           cl::init(true));

static cl::opt<bool>
ExternSData("mextern-sdata", cl::Hidden,
            cl::desc("EMBER: Use gp_rel for data that is not defined by the "
                     "current object."),
            cl::init(true));

static cl::opt<bool>
EmbeddedData("membedded-data", cl::Hidden,
             cl::desc("EMBER: Try to allocate variables in the following"
...
void EMBERTargetObjectFile::Initialize(MCContext &Ctx, const TargetMachine &TM){
  ...
  SmallDataSection = getContext().getELFSection(
      ...
      ELF::SHF_WRITE | ELF::SHF_ALLOC | ELF::SHF_EMBER_GPREL);

  SmallBSSSection = getContext().getELFSection(".sbss", ELF::SHT_NOBITS,
                                               ELF::SHF_WRITE | ELF::SHF_ALLOC |
                                                   ELF::SHF_EMBER_GPREL);
  this->TM = &static_cast<const EMBERTargetMachine &>(TM);
}
...
/// section.
bool EMBERTargetObjectFile::IsGlobalInSmallSection(
...
IsGlobalInSmallSectionImpl(const GlobalObject *GO,
  ...
  const EMBERSubtarget &Subtarget =
      *static_cast<const EMBERTargetMachine &>(TM).getSubtargetImpl();
...
MCSection *EMBERTargetObjectFile::SelectSectionForGlobal(
...
bool EMBERTargetObjectFile::IsConstantInSmallSection(
    const DataLayout &DL, const Constant *CN, const TargetMachine &TM) const {
  return (static_cast<const EMBERTargetMachine &>(TM)
              .getSubtargetImpl()
              ->useSmallSection() &&
...
/// Return true if this constant should be placed into small data section.
MCSection *EMBERTargetObjectFile::getSectionForConstant(const DataLayout &DL,
                                                       SectionKind Kind,
                                                       const Constant *C,
...
const MCExpr *
EMBERTargetObjectFile::getDebugThreadLocalSymbol(const MCSymbol *Sym) const {
  const MCExpr *Expr =
      MCSymbolRefExpr::create(Sym, MCSymbolRefExpr::VK_None, getContext());
  Expr = MCBinaryExpr::createAdd(
      Expr, MCConstantExpr::create(0x8000, getContext()), getContext());
  return EMBERMCExpr::create(EMBERMCExpr::MEK_DTPREL, Expr, getContext());
}
