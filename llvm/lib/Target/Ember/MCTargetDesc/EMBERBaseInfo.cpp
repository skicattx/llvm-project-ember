//===-- EMBERBaseInfo.cpp - Top level definitions for EMBER MC ------------===//
//
...
ABI computeTargetABI(const Triple &TT, FeatureBitset FeatureBits,
  ...
  if (!ABIName.empty() && TargetABI == ABI_Unknown) {
    ...
    // TODO: move this checking to EMBERTargetLowering and EMBERAsmParser
    errs()
...
MCRegister getBPReg() { return EMBER::X9; }

// Returns the register holding shadow call stack pointer.
MCRegister getSCSPReg() { return EMBER::X18; }
...
void validate(const Triple &TT, const FeatureBitset &FeatureBits) {
  if (TT.isArch64Bit() && !FeatureBits[EMBER::Feature64Bit])
    report_fatal_error("RV64 target requires an RV64 CPU");
  if (!TT.isArch64Bit() && FeatureBits[EMBER::Feature64Bit])
    report_fatal_error("RV32 target requires an RV32 CPU");
  if (TT.isArch64Bit() && FeatureBits[EMBER::FeatureRV32E])
    report_fatal_error("RV32E can't be enabled for an RV64 target");
}

} // namespace EMBERFeatures

void EMBERVType::printVType(unsigned VType, raw_ostream &OS) {
  EMBERVSEW VSEW = getVSEW(VType);
  EMBERVLMUL VLMUL = getVLMUL(VType);

  unsigned Sew = 1 << (static_cast<unsigned>(VSEW) + 3);
  OS << "e" << Sew;

  switch (VLMUL) {
  case EMBERVLMUL::LMUL_RESERVED:
    llvm_unreachable("Unexpected LMUL value!");
  case EMBERVLMUL::LMUL_1:
  case EMBERVLMUL::LMUL_2:
  case EMBERVLMUL::LMUL_4:
  case EMBERVLMUL::LMUL_8: {
    unsigned LMul = 1 << static_cast<unsigned>(VLMUL);
    OS << ",m" << LMul;
    break;
  }
  case EMBERVLMUL::LMUL_F2:
  case EMBERVLMUL::LMUL_F4:
  case EMBERVLMUL::LMUL_F8: {
