//===- EMBERMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
static void generateInstSeqImpl(int64_t Val, bool IsRV64,
  ...
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(EMBERMatInt::Inst(EMBER::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = (IsRV64 && Hi20) ? EMBER::ADDIW : EMBER::ADDI;
      Res.push_back(EMBERMatInt::Inst(AddiOpc, Lo12));
    }
    return;
  }
  ...
  Res.push_back(EMBERMatInt::Inst(EMBER::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(EMBERMatInt::Inst(EMBER::ADDI, Lo12));
}

namespace llvm {
namespace EMBERMatInt {
InstSeq generateInstSeq(int64_t Val, bool IsRV64) {
  ...
  if (Val > 0 && Res.size() > 2) {
    ...
    EMBERMatInt::InstSeq TmpSeq;
    generateInstSeqImpl(Val, IsRV64, TmpSeq);
    TmpSeq.push_back(EMBERMatInt::Inst(EMBER::SRLI, ShiftAmount));
